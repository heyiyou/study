...
//			update ps_board_free set b_title='bb',b_text='bbbb' where b_no=4;
	...
	public void reg(String id, String pw) {
	    try {
	        ...
	        // 적절한 회원 테이블과 컬럼명으로 수정임 
	        String sql = String.format("INSERT INTO member (s_id, s_pw) VALUES ('%s', '%s')", id, pw);
	        st.executeUpdate(sql);
	...
	public boolean login(String id, String pw) {
	    ...
	    try {
	        Class.forName(Db.DB_JDBC_DRIVER_PACKAGE_PATH);
	        con = DriverManager.getConnection(Db.DB_URL, Db.DB_ID, Db.DB_PW);   //상수정의
	        st = con.createStatement();     //SQL  statement객체 생성

	        // 회원 테이블 이름과 컬럼명에 맞게 수정하세요
