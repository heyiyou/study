...
<%
	...
	//이전 다음이란? ex.    <이전 [1][2][3][4][5] 다음> <<요런거  예를들어 내가 3에 있는데 다음을 누르면 6부터 시작 할수있는프로그램
	int prevPage = 0;
	int nextPage = 0;
	...
	boolean hasPrev = true;	//이전 블럭 가기 가능 여부 저장값 초기화.               이 이야기는 내가 12345 쪽에 있는데 이전을 막아놓는거 이유는 0,-1 뭐 이렇게 이동 못가니깐
	if(currentBlockNo == 1){	//현재 블럭이 1번 블럭이면
		...
		prevPage = (currentBlockNo - 1 ) * Board.PAGE_LINK_AMOUNT;      // 이코드는 내가 7이었는데 이전을 눌어 그럼 1번째 블럭에 막지막 번호 즉 5로 넘어가라고 하는 코드임
	}
	...
	if(currentBlockNo < totalBlock ){	//현재 블럭이 마지막 블럭보다 작으면
		...
		nextPage = currentBlockNo * Board.PAGE_LINK_AMOUNT + 1;      //이코드는 위에있는코드와의 반대임 12345 블럭에 있으면 다음을 눌렀다? 그럼 6으로		
