...
public class BoardListProcessor {
	...
	public BoardListProcessor(GuestMapper mapper, int currentPage) {
		super();
		this.mapper = mapper;
	...
	public String getHtmlPageList() {
		...
		if(hasPrev){
			html=html+String.format("<a href='/guest/getList?currentPage=%d'>🐿️이전블럭가기🐿️</a>",prevPage);
		}		
		...
		for(int i=blockStartNo;i<=blockEndNo;i++){	// 	<< 이렇게 바꿈
			html = html + String.format("<a href='/guest/getList?currentPage=%d'>%d</a>&nbsp;&nbsp;",i,i);
		}
		
		//🐿️🐿️🐿️{블럭 처리 - 9/9}.(이전/다음)의 (다음) 처리🐿️🐿️🐿️
		//🐿️🐿️🐿️: 다음 블럭 이동이 가능하면 미리 계산한 이전 블록 이동 시 이동 할 페이지번호를 랑크에 전달하기🐿️🐿️🐿️//
		if(hasNext){
			html=html+String.format("<a href='/guest/getList?currentPage=%d'>🐿️다음블럭가기🐿️</a>",nextPage);
		}		
		return html;
	}
}