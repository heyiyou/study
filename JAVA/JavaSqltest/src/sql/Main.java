...
public class Main {
	public static void main(String[] args) {
		initDb(); // 먼저 DB 세팅 (연결, 테이블 생성 등)
		dbRun();  // 이후 DB 관련 동작 수행 (CRUD 등)
	}
	...
	static private void dbRun() {
		try {	// 아래 코드들을 이 try{ 블럭 안에 안쓰면 에러남.
			...
			ResultSet result = st.executeQuery("select * from tottenham_squad where p_position = '공격수'");	//전송 할 sql 문  executeQuery카톡 보내기 역할   이거 (내생각) 중요!
